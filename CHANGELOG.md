...
- **Colour in the memory commands.** `read`, `search`, `recall` and `history`
  paint their structure when you are looking at a terminal: ids in amber,
  because those are what you retype into `--supersedes` and `history`; dates and
  tags dimmed; the leading glyph coloured by standing. The memory text is left
  plain, because colour there would compete with the one thing it already means
  - that a secret was masked or a private note withheld.
- **`voli memory stats` reports records it could not read**, and `doctor` says
  the same. A record that fails to decrypt is skipped rather than fatal - one
  bad record must not blind a whole store - but the skip was silent, so `read`,
  `search` and `stats` all under-reported while `verify` counted the full set.
  A store written under a different key is the usual cause.

### Note

Colour is applied at the CLI edge, never in the memory engine. The same rendered text
reaches an agent through the session-start hook, through MCP, and through a
pipe, and an escape code in any of those is context an agent pays for and a
parser can trip over. `NO_COLOR` and a non-terminal destination both switch it
off.
...
- `voli memory prompt` git-ignores the file it writes, and says the file is
  regenerated rather than kept. The prompt is generated by voli itself, so a
...
- **One containment rule, not three.** The rule telling an agent that memories
  ...
  vector entirely. There is now one canonical rule, with a test that
