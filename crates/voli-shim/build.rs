//! Windows-only: embed the bear icon + version-info metadata into both shim
//! binaries (voli-shim.exe and voli-shim-gui.exe). No-op on non-Windows.
...
fn main() {
    #[cfg(windows)]
    {
        ...
        if ico_path.exists() {
            ...
            res.set("FileDescription", "voli shim stub");
