...
//!
//! On Linux/macOS executables carry no PE icon resources and shims need no
//! picture — a `chmod +x` copy is enough — so this module is a no-op there.
...
mod stub {
    ...
    /// Unix: icons do not exist — always a clean no-op.
    pub fn copy_exe_icon(_target: &Path, _shim: &Path) -> io::Result<bool> {
        Ok(false)
    }

    /// Unix: no icon resources — always `None`.
    pub fn primary_group_icon(_exe: &Path) -> Option<Vec<u8>> {
        None
    }
}

#[cfg(not(windows))]
pub use stub::{copy_exe_icon, primary_group_icon};
...
#[cfg(windows)]
pub use windows_impl::{copy_exe_icon, primary_group_icon};
